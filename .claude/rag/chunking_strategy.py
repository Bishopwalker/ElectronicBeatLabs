...
class EBLChunkingStrategy:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    chunk_id = self._generate_chunk_id(str(file_path), f'class_{node.name}_L{node.lineno}')
            ...
            for node in tree.body:
                if isinstance(node, ast.FunctionDef):
                    ...
                    # Include line number for uniqueness
                    chunk_id = self._generate_chunk_id(str(file_path), f'func_{node.name}_L{node.lineno}')
    ...
    def _generate_chunk_id(self, file_path: str, chunk_identifier: str) -> str:
        ...
        # Use full 32-char hash to avoid collisions
