...
def initialize_memory(project_root_path: str):
    ...
    try:
        sys.path.insert(0, str(Path(project_root_path)))
        from .claude.rag import load_config, MemoryService
...
async def enhance_query(query: str, session_id: str) -> str:
    ...
    Example:
        Input: "buffer"
        Session Context: "Working on audio buffer starvation in WebSocket streaming"
        Output: "audio buffer starvation in WebSocket streaming buffer management"
