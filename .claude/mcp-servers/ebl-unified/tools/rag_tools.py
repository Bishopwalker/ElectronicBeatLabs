...
def initialize_rag(project_root_path: str):
    ...
    try:
        # Import from Agent 3's RAG system
        # Add .claude directory to path to import rag module
        claude_path = Path(project_root_path) / ".claude"
        sys.path.insert(0, str(claude_path))
        from rag import load_config, CloudRAGPipeline
        from rag.vector_database import VectorProvider

        _rag_config = load_config()

        # Convert string provider to enum
        provider_map = {
            "chroma": VectorProvider.CHROMA,
            "supabase": VectorProvider.SUPABASE,
            "pinecone": VectorProvider.PINECONE,
            "weaviate": VectorProvider.WEAVIATE
        }
        provider_enum = provider_map.get(_rag_config.vector_provider, VectorProvider.CHROMA)
